...
def setup_reimport_initialize(mocker, temp_products=None, temp_json=None):
    ...
    if temp_products is not None:
        mocker.patch('tasks_1_and_2.online_shopping_cart.product.product_data.get_products', return_value=deepcopy(temp_products))
...
def test_add_product_boundaries(user_input, expected_output, mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=user_input)
...
def test_print_stock(mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=['d', 'l', 'y'])
    setup_reimport_initialize(mocker, mock_global_products())
...
def test_checkout_empty_cart(mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=['c', 'l', 'y'])
    setup_reimport_initialize(mocker, mock_global_products())
    assert capsys.readouterr().out == ('\nItems in the cart:'
                                       '\nYou have been logged out.\n')
...
def test_add_product_no_stock(mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=['1', 'l', 'y'])
...
def test_remove_valid_product_from_cart(mocker, capsys):
    ...
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=side)
...
def test_invalid_product_decimal(mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=['1.5', 'l', 'y'])
    setup = setup_reimport_initialize(mocker, mock_global_products())
    assert setup.global_cart.is_empty()
...
def test_logout_empty_cart(mocker, capsys):
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_login.UserInterface.get_user_input", side_effect=['l', 'y'])
    setup = setup_reimport_initialize(mocker, mock_global_products())
    assert setup.global_cart.is_empty()
