...
def test_checkout_insufficient_funds(mocker, capsys):
    user = User(name="Bob", wallet=50.0)
    cart = ShoppingCart()
    cart.add_item(Product(name="Apple", price=60.0, units=1))
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_interface.UserInterface.get_user_input", return_value='wallet')
    checkout(user, cart)
    captured = capsys.readouterr()
...
def test_checkout_success_single_item(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Carol! Your remaining balance is 8.0' in captured.out
    assert user.wallet == 8.0
    assert cart.is_empty()


def test_checkout_exact_wallet_zero(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Dave! Your remaining balance is 0.0' in captured.out
    assert user.wallet == 0.0
    assert cart.is_empty()


def test_checkout_multiple_units_total(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Eve! Your remaining balance is 91.0' in captured.out
    assert user.wallet == 91.0
    assert cart.is_empty()


def test_checkout_multiple_products(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Frank! Your remaining balance is 37.5' in captured.out
    assert user.wallet == 37.5
    assert cart.is_empty()


def test_checkout_does_not_clear_on_insufficient_funds(mocker, capsys):
...
def test_checkout_total_price_with_units(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Henry! Your remaining balance is 12.0' in captured.out
    assert user.wallet == 12.0
    assert cart.is_empty()


def test_checkout_wallet_float_precision(mocker, capsys):
...
def test_checkout_prints_username_and_balance(mocker, capsys):
    ...
    checkout(user, cart)
    captured = capsys.readouterr()
    assert 'Thank you for your purchase, Jack! Your remaining balance is 10.0' in captured.out
...
def test_check_cart_checkout_yes(mocker, capsys):
    ...
    mocker.patch("tasks_1_and_2.online_shopping_cart.user.user_interface.UserInterface.get_user_input", side_effect=['y', 'wallet'])
    result = check_cart(user, cart)
    captured = capsys.readouterr()
